...
# ---------------------------------------------------------
# 4. Sidebar (Search & Nav) - Design from app design.py
# ---------------------------------------------------------
with st.sidebar:
...
# ---------------------------------------------------------
    ...
    if not model_loaded or not data_loaded:
        st.error("❌ ML system not loaded. Please check Settings page.")
        st.info("Ensure you have: best_model.pkl, scaler.pkl, label_encoders.pkl, feature_columns.pkl, and patient.csv in your directory.")
        st.stop()
    ...
    with st.expander("Predict LOS for Specific Patient"):
        ...
        with col1:
            ...
            unittype = st.selectbox("Unit Type", sorted(df_eicu["unittype"].dropna().astype(str).unique().tolist()))
        with col2:
            unitadmitoffset = st.number_input("Admit Offset (min)", value=0)
            unitdischargeoffset = st.number_input("Discharge Offset (min)", value=1440)
            unitdischargelocation = st.selectbox("Discharge Location", ["Home", "Skilled Nursing Facility", "Rehabilitation", "Other"])
...
# ---------------------------------------------------------
    ...
    with tab2:
        ...
        if not data_loaded or df_eicu is None:
            st.error("Critical: eICU data not loaded. Check Settings.")
            st.info("The system needs patient.csv file for ML predictions.")
            st.stop()
        
        if not model_loaded:
            st.warning("ML model not loaded. Predictions will be based on historical averages.")
        ...
        with c3:
            if data_loaded:
                ...
                unittype = st.selectbox("Assigned Unit Type", unittype_options, key="ml_unit")
            else:
                unittype = st.text_input("Unit Type", value="ICU", key="ml_unit")
        ...
        if model_loaded:
            ...
            rt_pred_los = unitdischargeoffset / 60.0  # Use input value as fallback
...
# ---------------------------------------------------------
    ...
    with col2:
        ...
        if data_loaded:
            st.success(f"✓ eICU Data Loaded ({len(df_eicu) if df_eicu is not None else 0} patients)")
        else:
            st.error("❌ Data Missing")
            st.caption("patient.csv required")
    
    st.markdown("---")
    st.markdown('<div class="section-header">Required Files</div>', unsafe_allow_html=True)
    
    required_files = [
        ('best_model.pkl', 'Machine Learning Model', model_loaded),
        ('scaler.pkl', 'Feature Scaler', model_loaded),
        ('label_encoders.pkl', 'Label Encoders', model_loaded),
        ('feature_columns.pkl', 'Feature Columns', model_loaded),
        ('patient.csv', 'eICU Patient Data', data_loaded),
        ('logo.png', 'Application Logo', os.path.exists("logo.png"))
    ]
    
    for filename, description, loaded in required_files:
    ...
    - **ML System**: {"Active" if model_loaded and data_loaded else "Limited"}
    """)